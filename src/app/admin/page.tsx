...
export default async function AdminPage({
  ...
  // 1. Подтягиваем глобальные настройки (твоя старая логика)
  const settings = await getSiteSettings();
  
  // 2. Подтягиваем контент для выбранной страницы (новая логика ИИ)
  const selectedRoute = searchParams.route || '/';
  const content = await getPageContent(selectedRoute) || {};

  return (
    <div className="min-h-screen bg-cream-dark py-16 text-chocolate space-y-12">
      ...
      <div className="mx-auto max-w-3xl bg-cream p-10 shadow-sm border border-chocolate/5">
        ...
        <p className="font-sans text-sm text-chocolate/50 mb-8">
          Базовые параметры SEO и индексации для всего домена.
        </p>
        ...
        <form action={updateSiteSettings} className="space-y-6">
          ...
          <div>
            <label className="block text-xs font-sans uppercase tracking-widest text-chocolate/70 mb-2">
              Глобальное описание (Description)
            </label>
            <textarea 
          ...
          <SubmitButton 
            defaultText="Сохранить глобальные настройки"
      ...
      <div className="mx-auto max-w-3xl bg-cream p-10 shadow-sm border border-chocolate/5">
        ...
        {/* Форма выбора страницы (обновляет URL) */}
        ...
        <form action={savePageContent} className="space-y-6">
          ...
          <SubmitButton 
            defaultText={`Сохранить контент для ${selectedRoute}`}
            loadingText="Запись..."
